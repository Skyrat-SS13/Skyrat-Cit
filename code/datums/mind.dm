...
/datum/mind/proc/equip_traitor(employer = "You, criminal elements, rival factions, or The Syndicate", silent = FALSE, datum/antagonist/uplink_owner)
	if(!current)
		return
	...
	if (!uplink_loc)
		...
		if(uplink_loc == R)
			...
			if(!silent)
				to_chat(traitor_mob, "[employer] has cunningly disguised a black market uplink as your [R.name]. Simply dial the frequency [format_frequency(R.traitor_frequency)] to unlock its hidden features.")
			...
			if(!silent)
				to_chat(traitor_mob, "[employer] has cunningly disguised a black market uplink as your [PDA.name]. Simply enter the code \"[PDA.lock_code]\" into the ringtone select to unlock its hidden features.")
			...
			if(!silent)
				to_chat(traitor_mob, "[employer] has cunningly disguised a black market uplink as your [P.name]. Simply twist the top of the pen [P.traitor_unlock_degrees] from its starting position to unlock its hidden features.")
