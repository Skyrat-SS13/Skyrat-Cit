...
/client/proc/set_client_age_from_db(connectiontopic)
	...
	if(!query_client_in_db.NextRow())
		if (CONFIG_GET(flag/panic_bunker) && !holder && !GLOB.deadmins[ckey] && !(ckey in GLOB.bunker_passthrough))
			...
			to_chat(src, "<span class='notice'>Hi! We temporarily have protection enabled that prevents new players from joining for whatever reason. Please try again later, especially when our server tag includes the word "Open". Check out our Discord Server if you want to ask a staff when the panic bunker will be lowered again or to join our community! <br> To gain full access to the Discover, read the rules and post a request in the #access-requests channel under the \"Landing Zone\" category in the Discord server linked here: <a href='https://discord.gg/6RpdCgR'>https://discord.gg/6RpdCgR</a></span>") //CIT CHANGE - makes the panic bunker disconnect message point to the discord
